...
// The subset of palette colors used in app-related images is be wider
// than what we allow for in illustration use (see below)
...
// Since illustrations are a part of WordPress.com’s and Jetpack’s visual identity,
// we prefer them over generic Blue and Green (which were designed for app use)
const PALETTE_ILLUSTRATION_COLORS = _.pickBy( PALETTE_APP_COLORS, ( colorValue, colorName ) => {
	return ! _.startsWith( colorName, 'Blue' ) && ! _.startsWith( colorName, 'Green' );
} );

// Making sure both sets contain only unique color values
// (the palette uses aliases for some colors)
